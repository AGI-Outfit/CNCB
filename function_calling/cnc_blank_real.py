...
class CNCBlank(LLMJsonTool):
        ...
        while 1:
            ...
            if output != "":
                ...
                while 1:
                    chat_output = self.chat(system=prompt_output_dict["system"], user=prompt_output_dict["user"])
                    json_outputs = self.json_parser(chat_output)
                    if len(json_outputs) == command_len:
                        break
                    else:
                        self.level_print("command_len != len(json_outputs), Retry!", 1)
    ...
    def move_end(self, position: list) -> str:
        ...
        self.robot_tcp.end_absmovecircle_jkrobot(p_len, radius, p_transition.tolist(), p_end.tolist(), 0.5, 5.0,
                                                 5.0)
